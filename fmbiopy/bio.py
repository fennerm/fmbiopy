...
from plumbum.cmd import bowtie2, samtools, wc, zcat
...
def suffix_is_one_of(x, suffixes, case_sensitive=False):
...
def is_fastq(filename):
    return suffix_is_one_of(filename, [".fq", ".fastq", ".fq.gz", ".fastq.gz"])


def is_fasta(filename):
    return suffix_is_one_of(filename, [".fa", ".fasta", ".fa.gz", ".fasta.gz"])
...
def count_reads(filename):
    ...
    if is_fastq(filename) or is_fasta(filename):
...
def to_fastq(bam, output_prefix, zipped=True):
    ...
    (
        samtools["fastq", bam]
        | bbmap_repair[
            "in=stdin.fastq", "out=stdout", "outs=" + reads["unpaired"]
        ]
        | bbmap_reformat[
            "in=stdin.fastq",
            "int=t",
